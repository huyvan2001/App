    ...
    func uploadTweet(caption: String,completion: @escaping(Bool)-> Void){
        ...
        Firestore.firestore().collection("tweets")
            ...
            .setData(data){ error in
                if let error = error {
                    print("DEBUG: Did not upload with error \(error.localizedDescription)")
                    completion(false)
                    return
                }
                completion(true)
...
extension TweetService {
    func likeTweets(_ tweet: Tweet, completion: @escaping() -> Void){
        guard let uid = Auth.auth().currentUser?.uid else { return
        }
    ...
    func fetchLikeTweets(forUid id: String , completion: @escaping([Tweet])->Void){
        var tweets = [Tweet]()
        guard let uid = Auth.auth().currentUser?.uid else { return}
        Firestore.firestore()
            .collection("users")
